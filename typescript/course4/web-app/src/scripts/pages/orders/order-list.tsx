        ...
        return (
            <tr key={order.orderID}>
                ...
                <td>
                    ...
                    <div className={StringHelper.isUndefinedOrWhitespace(order.address2) ? "hidden" : ""}>{order.address2}</div>
                    <div>{order.city}</div>
                </td>
                <td>
                ...
                <td>
                    ...
                    <div><b>Total:</b> {
                        NumberHelper.formatAsCurrency(order.foodItems.map(f => f.price).filter(price => price != null).reduce((a, b) => a! + b!, 0))
                    }</div>
